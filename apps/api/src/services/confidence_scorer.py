...
class ConfidenceScorer:
    ...
    def _calculate_quality_score(self, extracted_data: Dict[str, Any]) -> float:
        ...
        if spec_count > 0:
            ...
            spec_quality = detailed_specs / spec_count if spec_count > 0 else 0
            
            # 调整评分标准：1-2个规格也可以获得较高分数
            if spec_count >= 3:
                quality_indicators.append(min(1.0, spec_quality))
            elif spec_count >= 2:
                quality_indicators.append(min(0.9, spec_quality * 0.9))
            else:  # 1个规格
                quality_indicators.append(min(0.7, spec_quality * 0.7))
        else:
    ...
    def _calculate_specification_confidence(self, spec_data: Dict[str, Any]) -> float:
        ...
        confidence = 0.5  # 提高基础分数
        
        # 检查规格值的质量
        spec_value = spec_data.get('value', '')
        spec_unit = spec_data.get('unit', '')
        
        if spec_value:
            value_str = str(spec_value).strip()
            if value_str:
                confidence += 0.25
                
                # 数值型规格（更常见，置信度更高）
                if self._is_numeric_spec(value_str):
                    confidence += 0.15
                    
                # 有明确单位的数值规格
                if spec_unit and self._is_technical_unit(spec_unit):
                    confidence += 0.1
        ...
        # 检查参数名称的技术性
        param_name = spec_data.get('parameter', '')
        if param_name and self._is_technical_parameter(param_name):
            confidence += 0.1
    ...
    def _is_numeric_spec(self, value_str: str) -> bool:
        """判断是否为数值型规格"""
        # 移除常见的单位和符号，检查是否包含数字
        cleaned_value = re.sub(r'[VvAaWwHh℃℉%Ω\-~±/:]', '', value_str)
        return bool(re.search(r'\d+', cleaned_value))
    
    def _is_technical_unit(self, unit_str: str) -> bool:
        """判断是否为技术单位"""
        technical_units = [
            'V', 'A', 'W', 'kW', 'MW', 'kV', 'mV', 'mA', 'Hz', 'kHz', 'MHz',
            '℃', '℉', 'K', '%', 'Ω', 'kΩ', 'MΩ', 'F', 'μF', 'nF', 'pF',
            'H', 'mH', 'μH', 'dB', 'dBm', 'mm', 'cm', 'm', 'km', 'kg', 'g',
            'bar', 'psi', 'Pa', 'kPa', 'MPa', 's', 'ms', 'μs', 'ns',
            'bit', 'byte', 'KB', 'MB', 'GB', 'TB', 'bps', 'Mbps', 'Gbps'
        ]
        unit_lower = unit_str.lower()
        return any(unit.lower() in unit_lower for unit in technical_units)
    
    def _is_technical_parameter(self, param_name: str) -> bool:
        """判断是否为技术参数名称"""
        technical_keywords = [
