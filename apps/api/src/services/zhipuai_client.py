...
**📊 专业术语智能识别与OCR修正：**
- OCR常见错误修正：将"O"→"0"、"l"→"1"、"S"→"5"、"TEST1"可能是"测试1"
- 通信协议识别："3RS232"→"RS232通信"、"WIRE1"→"线缆1"或"串口1"
- 技术简写：AC/DC、CT/PT、RMS、THD、FFT、GPS、PPS、DI/DO、AI/AO
- 单位变换：kV↔V、kA↔A、MW↔kW↔W、Hz、Ω、°、% 
- 比值格式：220/110、10000/100、35/0.4、1:1等电力设备常见比值
- 混合语言处理："/λspec_table"→"规格表"、"ToC50900608"→"产品型号ToC50900608"

**🚫 严格排除以下格式噪声和无效内容（绝不提取）：**
1. **页面元素**: "PAGE 7"、"第7页"、页码、章节号、目录标记
2. **Word格式标记**: "HYPERLINK"、"EMBED"、"MERGEFORMAT"、超链接标记、"λspec_table中提取"、"/λspec_table中提取"
3. **表格边框符号**: "A A AB X B"、"Ca a a a b"、管道符、连字符分隔线
4. **文档结构噪声**: 目录项、索引项、标题编号、书签标记、"附录A"、"技术规格详表"
5. **OCR错误识别**: 无意义字符组合、破碎的表格边框、格式化符号
6. **单个字母或数字**: 除非明确是技术参数（如V、A、Hz等单位），否则忽略单独的字符如"D"、"I"、"RS"
7. **混合语言噪声**: 含有特殊符号的混合文本如"λspec_"、"/\\"等格式标记
8. **数字字母组合噪声**: "3.2 D"、"5.2.14 I-t"、"I-t"等无技术意义的数字字母组合
9. **产品型号变体错误**: 对于相似产品型号(如ToC509006008、ToC509006048)，只保留文档中明确标注的正确型号
10. **基本信息混淆**: 产品名称、制造商、产品类别等基本信息不要放入技术规格中

**🔍 数据质量自我验证规则（提取前必须检查）：**
- 每个提取的规格参数必须有明确的技术含义
- 规格值必须包含数字、单位或技术描述
- 如果某项看起来像格式噪声（如"TEST"、"RS"、"D"单独出现），宁可不提取也不要包含
- 置信度评分时，发现格式噪声应大幅降低相应字段的置信度
- 对于疑似OCR错误的内容，尝试智能修正而非直接忽略

**⚠️ 提取前强制检查清单（违反直接跳过）：**
❌ 绝对不要提取以下内容到specifications中：
1. 任何 "ToC" 开头的产品型号变体（除非是文档明确的正确型号）
2. 任何纯数字+字母组合如 "3.2 D"、"5.2.14 I-t"
3. 任何单独的字母如 "D"、"I"、"RS"
4. 任何包含 "λspec" 或 "/λ" 的格式标记
5. 任何 "附录"、"表格"、"图表" 等文档结构内容
6. 任何产品基本信息如产品名称、制造商、类别等

**🎯 强制检查方法：**
在加入specifications之前，对每个参数名问自己：
- 这是真正的技术规格吗？
- 这有具体的技术数值或单位吗？
- 这不是文档格式或OCR噪声吗？
- 这不是产品基本信息吗？
...

{
  ...
  "specifications": {
    ...
    "通信接口": {"value": "接口类型", "unit": "", "description": "通信接口规格（如RS232、RS485、以太网等）"},
    "通信协议": {"value": "协议类型", "unit": "", "description": "支持的通信协议（如Modbus、IEC61850等）"},
    "数据传输速率": {"value": "速率值", "unit": "bps/kbps", "description": "数据传输速率"},
    "显示屏": {"value": "显示规格", "unit": "", "description": "显示屏幕规格"},
    "操作界面": {"value": "界面类型", "unit": "", "description": "人机交互界面类型"},
    "外形尺寸": {"value": "尺寸规格", "unit": "mm", "description": "设备外形尺寸（长×宽×高）"},
    "重量": {"value": "重量值", "unit": "kg", "description": "设备净重量"},
    "安装方式": {"value": "安装类型", "unit": "", "description": "设备安装方式（台式/壁挂/导轨等）"},
    "存储介质": {"value": "存储类型", "unit": "", "description": "数据存储介质和容量"}
  },
  "features": [
...
8. **通信协议智能识别**：
   - RS232/RS485/以太网接口识别："3RS232"→"RS232"、"WIRE1 3RS232"→"RS232接口1"
   - 协议名称修正：ModBus、IEC61850、DNP3.0等电力行业标准协议
   - 传输速率：9600bps、115200bps、10Mbps等标准速率
...
11. **智能参数分组**：
    ...
    - 接口参数：通信接口、显示界面、操作方式
12. **OCR错误智能修正**：
    - 型号修正："ToC50900608"可能是产品型号，保持原样
    - 数值修正："5.2.14 I-t"可能是"5.2.14 A-t"（电流-时间特性）
    - 单位补全："75"配合上下文可能是"75Hz"、"75V"或"75A"
...
- ✅ 是否包含混合语言噪声（如"λspec_table中提取"）？如有，必须删除
- ✅ 单独的字母或无意义组合（如"TEST"、"RS"、"D"）？如有，必须删除
- ✅ OCR错误是否已智能修正（如"3RS232"→"RS232通信接口"）？
- ✅ 技术参数是否包含数值和单位？缺乏具体信息的参数应删除
- ✅ 置信度评分是否合理？发现噪声数据应降低评分
- ✅ JSON格式是否完全符合要求？

**🔍 特殊验证规则：**
- 通信接口字段：必须是标准接口名称（RS232、RS485、以太网、USB等），不能是"TEST"、"RS"等无意义内容
- 数值参数：必须包含数字和单位，如"220V"、"50Hz"，不能是单独的"75"、"D"等
- 产品型号：应保持完整格式，如"ToC50900608"，但要确保是真实型号
- 技术规格：每个参数都应有明确的技术含义和应用场景

请深度分析文档内容，严格执行噪声过滤和智能修正，提取所有可识别的技术信息。特别注意识别和修正OCR错误，将模糊的技术信息转换为标准化的规格参数。只返回JSON格式结果，不包含任何其他文字。"""
